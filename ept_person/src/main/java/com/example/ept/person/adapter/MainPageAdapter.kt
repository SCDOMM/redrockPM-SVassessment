...
class MainPageAdapter : ListAdapter<UserHomeItem, RecyclerView.ViewHolder>(MainPageDiffCallback) {
    ...
    override fun onCreateViewHolder(
        ...
        return when (viewType) {
            ...
            TYPE_VIDEO_RECENT -> {
                val view =
                    inflater.inflate(R.layout.item_recent, parent, false)
    ...
    inner class SectionTitleViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        val tvItemItemTitle: TextView = itemView.findViewById(R.id.tv_item_item_title)
        fun bindData(item: UserHomeItem.Title) {
            tvItemItemTitle.text = item.text
        }
    }

    inner class RecentVideoViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        val rvRecentItem: RecyclerView = itemView.findViewById(R.id.rv_recent_item)
        val recentAdapter = RecentAdapter()
        fun bindData(item: UserHomeItem.SlideVideoGroup) {
            rvRecentItem.adapter = recentAdapter
            val manager =
                LinearLayoutManager(itemView.context, LinearLayoutManager.HORIZONTAL, false)
            rvRecentItem.layoutManager = manager
            rvRecentItem.onFlingListener = null
    ...
    inner class PopularVideoViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        fun bindData(item: UserHomeItem.SingleContent) {
            ...
            tvVideoNameItem.text = data.title ?: data.video?.title ?: ""
            ...
            tvVideoFavorItem.text = data.consumption?.collectionCount?.toString() ?: "0"
    ...
    inner class AlbumViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        fun bindData(item: UserHomeItem.AlbumCard) {
            ...
            previewList.forEach { video ->
                ...
                val ivCover = videoView.findViewById<ImageView>(R.id.iv_album_cover_item_item)
...
object MainPageDiffCallback : DiffUtil.ItemCallback<UserHomeItem>() {
    override fun areItemsTheSame(oldItem: UserHomeItem, newItem: UserHomeItem): Boolean {
        return when (oldItem) {
            ...
            is UserHomeItem.SlideVideoGroup if newItem is UserHomeItem.SlideVideoGroup ->
                oldItem.videoItems == newItem.videoItems  // 注意：List equality 需要重写 WorkMetroData 的 equals
    ...
    override fun areContentsTheSame(oldItem: UserHomeItem, newItem: UserHomeItem): Boolean {
        return oldItem == newItem  // 如果 data class 自动生成 equals，则可直接比较
    }
}